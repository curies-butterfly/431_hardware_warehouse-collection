...
static error_status write_flash_data_no_cheack(uint32_t write_address, const uint8_t * p_buffer, uint16_t size)
{
...
error_status write_flash_data(uint32_t write_address, const uint8_t * p_buffer, size_t size)
{
    ...
    while(1)
    {
        ...
        {
            if(g_flash_buffer[sector_offset + i] != 0xFF)
                break;
        }
        ...
        if(i < sector_remain)
        {
            ...
            if(write_flash_data_no_cheack(sector_position * SECTOR_SIZE + FLASH_BASE, g_flash_buffer, SECTOR_SIZE) != SUCCESS)
                return ERROR;
        }
        else
        {
            if(write_flash_data_no_cheack(write_address, p_buffer, sector_remain) != SUCCESS)
                return ERROR;
        }
...
const struct fal_flash_dev at32f4_onchip_flash =
{
    .name       = "onchip_flash",
    .addr       = 0x08000000,
    .len        = 1024*1024,
