...
float Power_Buff1[Diff_Samples_Num]={0,};//功率数组1
        ...
        if(time_diff >= 50 || is_first_run)
        {
            ...
            {   
                ...
                if(P_change1 > 25)
                {
                    feed_num++;
                    // 优化建议：确保 P_total1 变量名正确
                    online_cluster_update(&oc, P_total2); 
                }
            ...
            else if(oc.learning_mode == 0) // 使用 else if 结构更清晰
            {
                // 检测模式逻辑
                // 3. 性能优化：只调用一次检测函数，用变量存储结果
                int check_result = online_cluster_is_abnormal(&oc, P_total2);
...
#endif
static uint32 last_learn_time = 0;
#if III
        // 1. 声明为 volatile，确保编译器每次都从内存读取最新值，而不是使用寄存器缓存
        extern volatile Uint16 TIM0_Cnt_Base; 
        ...
        if(time_diff >= 50 || is_first_run)
        {
            ...
            else if(oc.learning_mode == 0) // 使用 else if 结构更清晰
            {
                // 检测模式逻辑
                // 3. 性能优化：只调用一次检测函数，用变量存储结果
                int check_result = online_cluster_is_abnormal(&oc, P_total3);
