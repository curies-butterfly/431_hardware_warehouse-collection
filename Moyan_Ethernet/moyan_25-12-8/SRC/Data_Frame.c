...
/*void SCI_SendHeart()
{
	s_ucTxBuf[0]=HRT_FLAG_H;     //起始位  0xAA 0xEE
	s_ucTxBuf[1]=HRT_FLAG_L;
	s_ucTxBuf[2]=HRT_CNT;        //长度位
	s_ucTxBuf[3]=DevID_L8;    //终端号低8位
	s_ucTxBuf[4]=DevID_L16;    //终端号低16位
	s_ucTxBuf[5]=DevID_H8;    //终端号高8位
	s_ucTxBuf[6]=DevID_H16;    //终端号高16位
	//s_ucTxBuf[7]=0x0; //终端号(低字节)
...
/**
 * @brief 发送聚类数据包（最近中心索引和距离）
 * 输入：无
 * 返回：无
 */
void LAN_Cluster(void)
{
	// 清空缓冲区相关部分
	s_ucTxBuf[0] = JUDGE_FLAG_H;     // 包头高8位 0xFC
	s_ucTxBuf[1] = JUDGE_FLAG_L;     // 包头低8位 0xFC
	s_ucTxBuf[2] = JUDGE_CNT;        // 包长度
	...
	// 发送最近中心索引 (uint8类型)
	s_ucTxBuf[7] = judge_nearest_idx;
	
	// 发送最近中心距离 (uint16类型，分高低字节)
	s_ucTxBuf[8] = (u8)(judge_nearest_dist & 0x00FF);      // 距离低8位
	s_ucTxBuf[9] = (u8)((judge_nearest_dist >> 8) & 0x00FF); // 距离高8位
	...
	// 发送数据
	send(SOCK_TCPS, s_ucTxBuf, JUDGE_CNT);
}
