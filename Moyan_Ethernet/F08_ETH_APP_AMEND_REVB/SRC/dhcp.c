...
uint8    DHCP_SIP[4] = {0,0,0,0};      // DNS server ip address is discovered
uint8    DHCP_REAL_SIP[4] = {0,0,0,0}; // For extract my DHCP server in a few DHCP servers
...
/*
*********************************************************************************************************
*              SEND DHCP DISCOVER客户端请求包
*
* Description: This function sends DHCP DISCOVER message to DHCP server.
               这个报文是client端开始dhcp过程的第一个请求报文，client在请求地址时，
               并不知道server端的位置，所以client会以广播的方式发送请求报文，
               它的目的是发现网络中的服务器。
...
void send_DHCP_DISCOVER(void)
{
	uint8 ip[4] = {255,255,255,255};
		...
		pRIPMSG->xid[0] = (uint8)((htonl(DHCP_XID)>>0)&0xFF);
		pRIPMSG->xid[1] = (uint8)((htonl(DHCP_XID)>>8)&0xFF);
		pRIPMSG->xid[2] = (uint8)((htonl(DHCP_XID)>>16)&0xFF);
		pRIPMSG->xid[3] = (uint8)((htonl(DHCP_XID)>>24)&0xFF);
		pRIPMSG->secs[0] = (uint8)(htons(DHCP_SECS)>>0)&0xFF;
		pRIPMSG->secs[1] = (uint8)(htons(DHCP_SECS)>>8)&0xFF;
		pRIPMSG->flags[0] =(uint8)(htons(DHCP_FLAGSBROADCAST)>>0)&0xFF;
		pRIPMSG->flags[1] =(uint8)(htons(DHCP_FLAGSBROADCAST)>>8)&0xFF;
		...
		//for(uint8 i=0; i<3; i++)
		//Delay_ms(800);
		//sendto1(SOCK_DHCP, (uint8 *)pRIPMSG, sizeof(RIP_MSG), ip, DHCP_SERVER_PORT);
		//while(1);
...
void send_DHCP_REQUEST(void)
{
  ...
  //pRIPMSG->secs = htons(DHCP_SECS);
  		pRIPMSG->secs[0] = (uint8)(htons(DHCP_SECS)>>IS8)&0xFF;
  				pRIPMSG->secs[1] = (uint8)(htons(DHCP_SECS)>>IS0)&0xFF;

  if(dhcp_state < STATE_DHCP_LEASED)
  {
    //pRIPMSG->flags = htons(DHCP_FLAGSBROADCAST);
	  pRIPMSG->flags[0] =(uint8)(htons(DHCP_FLAGSBROADCAST)>>IS8)&0xFF;
	  		pRIPMSG->flags[1] =(uint8)(htons(DHCP_FLAGSBROADCAST)>>IS0)&0xFF;
  }
  else
  {
    pRIPMSG->flags[0] = 0;
    pRIPMSG->flags[1] = 0;// For Unicast
...
*********************************************************************************************************
*/
uint32 ID_trans(uint8 *buf)
{
	uint32 ret = 0;
	ret = buf[3]*16777216 + buf[2]*65536 + buf[1]*256 + buf[0];
	return ret;
}
uint8 parseDHCPMSG(uint16 length)
{
  ...
  if(memcmp(pRIPMSG->chaddr,SRC_MAC_ADDR,6) != 0 || ID_trans(pRIPMSG->xid) != htonl(DHCP_XID))
  {
    #ifdef DHCP_DEBUG
     ...
     // printf("\tpRIPMSG->xid(%08X), DHCP_XID(%08X)",ID_trans(pRIPMSG->xid,(DHCP_XID));
 ...
 // printf("e : %08d\n",(uint32)e);
  ...
  while ( p < e )
  {
    ...
    case dhcpIPaddrLeaseTime :
    	opt_len = *p++;
	    //时间错误 2021年8月8日21:22:24 modify zll 修正
	    dhcp_lease_time  = *p++;
...
//#ifdef DHCP_DEBUG
	  ...
	  SCI_Printf("dhcpIPaddrLeaseTime : %d\r\n", (int)dhcp_lease_time);
//#endif
      break;
...
* Arguments  : None.
* Returns    : None.
* Note       :因为dsp最小存储单位是16位，如sizeof（int）==1，故所算出的DHCP报文数据长度不正确，因此结构体内数据全改成了U8，
              可实现基本的IP获取，但有如下bug
	            DHCP流程 ：发discover--收offer--发request--收ack(受阻)
	            此处有个  bug 发送request后收到的ACK并不能正确解析，但实际IP网关等已经获取到，且做了IP冲突检测。后续需要优化
	            于 2021年8月9日19:46:31解决 BY zll
*********************************************************************************************************
*/
uint8 check_DHCP_state(SOCKET s) 
{
  ...
  {
	  /*if(dhcp_state == STATE_DHCP_REQUEST)
		  goto out1;*/
...
void dhcp_set_ip()
{
	...
	printf("%u.%u.%u.%u\r\n", DHCP_GET.dns[0], DHCP_GET.dns[1], DHCP_GET.dns[2], DHCP_GET.dns[3]);*/
...
void do_dhcp()
{
	...
	case DHCP_RET_UPDATE:														 /*成功获取到IP地址*/
		dhcp_set_ip();                               /*将获取到的IP地址写入W5500寄存器*/
		printf("\r\nDHCP服务器获得IP地址\r\n");
