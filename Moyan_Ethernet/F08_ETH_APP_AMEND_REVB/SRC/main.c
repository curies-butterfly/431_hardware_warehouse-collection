...
#define POST_SHIFT   0  // Shift results after the entire sample table is full 当整个样本表填满后，转移结果。
#define INLINE_SHIFT 1  // Shift results as the data is taken from the results regsiter 当整个样本表填满后，转移结果。
...
/* 
 *@brief MemCopyUser 是个简化版 memcpy，但专门针对 Uint16 类型。
 *		 它会从 SourceAddr 拷贝到 SourceEndAddr-1，写入 DestAddr 开始的区域，
 *		 常用于启动代码中把 Flash 里的函数/数据搬运到 RAM。
 */
 ...
 */
InitSysCtrl();
InitXintf16Gpio();// Xintf总线IO初始化

/*初始化串口SCI-A的GPIO*/ 
InitSciaGpio();

/**
 * @简述:EALLOW，EDIS是成对使用的，有些寄存器是受到保护的，不能任意写，
 *       EALLOW相当于去掉保护，对写保护的寄存器进行操作后 EDIS 是重新把这个寄存器保护起来的意思。
 * @示例：
		EALLOW;                   允许写受保护寄存器
		SysCtrlRegs.WDCR = 0x68;  修改看门狗寄存器
		EDIS;                     禁止写受保护寄存器
 */
EALLOW;
...
DINT;//清除所有中断初始化中断向量表: 禁止CPU全局中断
 ...
 /**
  * @brief 初始化PIE中断向量表，并使其指向中断服务子程序（ISR）
...
//EALLOW相当于去掉保护，对写保护的寄存器进行操作后 EDIS 是重新把这个寄存器保护起来的意思。
   ...
   PieVectTable. EPWM1_INT = &epwm_int; // pwm波初始化
   PieVectTable. TINT0     = &ISRTimer0;  //将定时器中断添加都中断向量表里
   PieVectTable. XINT13    = &ISRTimer1;
   PieVectTable.SCIRXINTA  = &sciaRxIsr;// 串口中断
   ...
   InitCapl();		//初始化CAP的相关配置
   InitAdc();		//初始化ADC
   ADC_Setup();     //ADC配置
   EPwmSetup();     //epwm设置
   scia_echoback_init(); // 初始化SCI-A工作方式和参数配置
...
/*********************************EEPROM*************************************/
    ...
    i = AT24CXX_ReadData(BL_Status_ADDRESS);			   //从EEPROM 指定地址读取一个字节的数据
    ClrAPP_Cnt();//clr app cnt
    Read_BoardInfo(); //读取ID

    if(i == BL_APP1) scia_msg((char*)app1StartBuff, sizeof(app1StartBuff)); //复位时发送数据
    if(i == BL_APP2) scia_msg((char*)app2StartBuff, sizeof(app2StartBuff)); //复位时发送数据
